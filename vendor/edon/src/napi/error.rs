...
impl From<JsUnknown> for Error {
    fn from(value: JsUnknown) -> Self {
        ...
        let status = unsafe {
            libnode_sys::napi_create_reference(value.0.env, value.0.value, 1, &mut result)
        };
...
macro_rules! impl_object_methods {
    ($js_value:ident, $kind:expr) => {
        impl<S: AsRef<str>> $js_value<S> {
            ...
            pub unsafe fn into_value(self, env: libnode_sys::napi_env) -> libnode_sys::napi_value {
                if !self.0.maybe_raw.is_null() {
                    let mut err = ptr::null_mut();
                    let get_err_status = unsafe {
                        libnode_sys::napi_get_reference_value(env, self.0.maybe_raw, &mut err)
                    };
                    debug_assert!(
                    ...
                    let is_error_status =
                        unsafe { libnode_sys::napi_is_error(env, err, &mut is_error) };
                    debug_assert!(
                ...
                let create_error_status =
                    unsafe { $kind(env, error_code, reason_string, &mut js_error) };
            ...
            ///
            /// This function is safety if env is not null ptr.
            pub unsafe fn throw_into(self, env: libnode_sys::napi_env) {
                ...
                assert_eq!(
                    unsafe {
                        libnode_sys::napi_is_exception_pending(env, &mut is_pending_exception)
                    },
                ...
                let js_error = match is_pending_exception {
                    true => {
                        ...
                        assert_eq!(
                            unsafe {
                                libnode_sys::napi_get_and_clear_last_exception(
                                    env,
                                    &mut error_result,
                                )
                            },
        ...
        match c {
            ...
            _ => {
                ...
                let error_msg = match value_type {
                    ValueType::Function => {
                        let function_name =
                            unsafe { JsFunction::from_raw_unchecked($env, $val).name()? };
                        format!(
                            $msg,
                            format!(
                                "function {}(..) ",
                    ...
                    ValueType::Object => {
                        ...
                        let object =
                            json.stringify($crate::napi::JsObject($crate::napi::Value {
                                value: $val,
                                env: $env,
                    ...
                    ValueType::Boolean | ValueType::Number => {
                        let value = unsafe {
                            $crate::napi::JsUnknown::from_raw_unchecked($env, $val)
                                .coerce_to_string()?
                        }
                        .into_utf8()?;
