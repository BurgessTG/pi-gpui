...
impl JsBigInt {
    ...
    pub fn coerce_to_number(self) -> Result<JsNumber> {
        ...
        check_status!(unsafe {
            libnode_sys::napi_coerce_to_number(self.raw.env, self.raw.value, &mut new_raw_value)
        })?;
        Ok(JsNumber(Value {
            env: self.raw.env,
            value: new_raw_value,
    ...
    pub fn coerce_to_string(self) -> Result<JsString> {
        let mut new_raw_value = ptr::null_mut();
        check_status!(unsafe {
            libnode_sys::napi_coerce_to_string(self.raw.env, self.raw.value, &mut new_raw_value)
        })?;
        Ok(JsString(Value {
            env: self.raw.env,
            value: new_raw_value,
    ...
    pub fn coerce_to_object(self) -> Result<JsObject> {
        let mut new_raw_value = ptr::null_mut();
        check_status!(unsafe {
            libnode_sys::napi_coerce_to_object(self.raw.env, self.raw.value, &mut new_raw_value)
        })?;
        Ok(JsObject(Value {
            env: self.raw.env,
            value: new_raw_value,
...
impl NapiValue for JsBigInt {
    unsafe fn from_raw(env: libnode_sys::napi_env, value: libnode_sys::napi_value) -> Result<Self> {
        let mut word_count = 0usize;
        check_status!(unsafe {
            libnode_sys::napi_get_value_bigint_words(
                env,
                value,
                ptr::null_mut(),
                &mut word_count,
