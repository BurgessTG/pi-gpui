        ...
        impl $js_value {
                ...
                T: FromNapiValue + ValidateNapiValue,
            {
                ...
                unsafe { <T as ValidateNapiValue>::validate(self.0.env, raw_value) }.map_err(
                    |mut err| {
                        err.reason =
                            format!("Object property '{name}' type mismatch. {}", err.reason);
                        err
                    },
                )?;
                ...
                S: NapiRaw,
            {
                let mut result = false;
                check_status!(unsafe {
                    libnode_sys::napi_delete_property(
                        self.0.env,
                        self.0.value,
                        name.raw(),
                        &mut result,
            ...
            pub fn has_own_property(&self, key: &str) -> Result<bool> {
                ...
                check_status!(unsafe {
                    libnode_sys::napi_has_own_property(
                        self.0.env,
                        self.0.value,
                        js_key,
                        &mut result,
