...
impl AsyncWorkPromise {
    ...
    pub fn cancel(&self) -> Result<()> {
        ...
        check_status!(unsafe {
            libnode_sys::napi_cancel_async_work(self.env, self.napi_async_work)
        })
    }
}
...
pub fn run<T: Task>(
    ...
    check_status!(unsafe {
        libnode_sys::napi_create_promise(env, &mut deferred, &mut raw_promise)
    })?;
...
unsafe extern "C" fn execute<T: Task>(_env: libnode_sys::napi_env, data: *mut c_void) {
...
unsafe extern "C" fn complete<T: Task>(
    ...
    if status != libnode_sys::Status::napi_cancelled && work.status.load(Ordering::Relaxed) != 2 {
            ...
            .and_then(|v| unsafe { ToNapiValue::to_napi_value(env, v) })
        {
            ...
            Err(e) => {
                let status = unsafe {
                    libnode_sys::napi_reject_deferred(
                        env,
                        deferred,
