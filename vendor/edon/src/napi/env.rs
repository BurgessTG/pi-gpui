...
use crate::napi::async_cleanup_hook::AsyncCleanupHook;
use crate::napi::async_work::AsyncWorkPromise;
use crate::napi::async_work::{self};
use crate::napi::bindgen_runtime::FromNapiValue;
use crate::napi::bindgen_runtime::ToNapiValue;
use crate::napi::check_status;
use crate::napi::cleanup_env::CleanupEnvHook;
use crate::napi::cleanup_env::CleanupEnvHookData;
#[cfg(feature = "serde-json")]
use crate::napi::js_values::De;
#[cfg(feature = "serde-json")]
...
use crate::napi::ExtendedErrorInfo;
use crate::napi::JsError;
use crate::napi::NodeVersion;
...
impl Env {
    ...
    pub fn create_bigint_from_i64(&self, value: i64) -> Result<JsBigInt> {
        let mut raw_value = ptr::null_mut();
        check_status!(unsafe {
            libnode_sys::napi_create_bigint_int64(self.0, value, &mut raw_value)
        })?;
    ...
    pub fn create_buffer(&self, length: usize) -> Result<JsBufferValue> {
        ...
        Ok(JsBufferValue::new(
            ...
            mem::ManuallyDrop::new(unsafe {
                Vec::from_raw_parts(data_ptr as *mut _, length, length)
            }),
        ))
    }
    ...
    pub fn adjust_external_memory(&mut self, size: i64) -> Result<i64> {
        ...
        check_status!(unsafe {
            libnode_sys::napi_adjust_external_memory(self.0, size, &mut changed)
        })?;
        Ok(changed)
    }
        ...
        D: AsRef<[u8]>,
    {
        ...
        Ok(JsBufferValue::new(
            ...
            mem::ManuallyDrop::new(unsafe {
                Vec::from_raw_parts(copy_data as *mut u8, length, length)
            }),
        ))
    }
    ...
    where
        F: 'static + Fn(crate::napi::CallContext<'_>) -> Result<R>,
        R: ToNapiValue,
    {
    ...
    pub fn fatal_error(self, location: &str, message: &str) {
        ...
        let location = CString::new(location)
            .expect(format!("Convert [{location}] to CString failed").as_str());
        let message =
    ...
    pub fn unwrap<T: 'static>(&mut self, js_object: &JsObject) -> Result<&mut T> {
        unsafe {
    ...
    pub fn get_value_external<T: 'static>(&mut self, js_external: &JsExternal) -> Result<&mut T> {
        unsafe {
            let mut unknown_tagged_object = ptr::null_mut();
    ...
    pub fn create_error(&self, e: Error) -> Result<JsObject> {
        ...
        let mut result = ptr::null_mut();
        check_status!(unsafe {
            libnode_sys::napi_create_error(
                self.0,
                ptr::null_mut(),
                reason_string.0.value,
                &mut result,
            )
        })?;
    ...
    {
        check_status!(unsafe {
            libnode_sys::napi_remove_env_cleanup_hook(
                self.0,
                Some(cleanup_env::<T>),
                hook.0 as *mut _,
            )
        ...
        F: FnOnce(FinalizeContext<T, Hint>),
    {
        check_status!(unsafe {
            libnode_sys::napi_set_instance_data(
                self.0,
                Box::leak(Box::new((TaggedObject::new(native), finalize_cb)))
                    as *mut (TaggedObject<T>, F) as *mut c_void,
                Some(
    ...
    if !finalize_hint.is_null() {
        ...
        if let Some(changed) = size_hint {
            if changed != 0 {
                ...
                let status = unsafe {
                    libnode_sys::napi_adjust_external_memory(env, -changed, &mut adjusted)
                };
                debug_assert!(
...
pub(crate) unsafe extern "C" fn trampoline<
    R: ToNapiValue,
    F: Fn(crate::napi::CallContext) -> Result<R>,
>(
    raw_env: libnode_sys::napi_env,
...
>(
    ...
    let env = unsafe { Env::from_raw(raw_env) };
    closure(env, unsafe {
        crate::napi::bindgen_runtime::Object::from_raw_unchecked(raw_env, raw_this)
    })
    .and_then(|ret: R| unsafe { <R as ToNapiValue>::to_napi_value(env.0, ret) })
    .unwrap_or_else(|e| {
        unsafe { JsError::from(e).throw_into(raw_env) };
