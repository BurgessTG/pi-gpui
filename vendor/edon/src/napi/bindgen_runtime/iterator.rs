...
pub fn create_iterator<T: Generator>(
    ...
    check_status_or_throw!(
        env,
        unsafe {
            libnode_sys::napi_set_property(env, instance, iterator_symbol, generator_function)
        },
...
pub unsafe extern "C" fn symbol_generator<T: Generator>(
    ...
    check_status_or_throw!(
        env,
        unsafe {
            libnode_sys::napi_define_properties(env, generator_object, 1, properties.as_ptr())
        },
...
extern "C" fn generator_next<T: Generator>(
    ...
    check_status_or_throw!(
        env,
        unsafe {
            libnode_sys::napi_set_named_property(env, result, c"done".as_ptr(), completed_value)
        },
...
extern "C" fn generator_return<T: Generator>(
    ...
    if argc > 0 {
        check_status_or_throw!(
            env,
            unsafe {
                libnode_sys::napi_set_named_property(env, result, c"value".as_ptr(), argv[0])
            },
    ...
    check_status_or_throw!(
        env,
        unsafe {
            libnode_sys::napi_set_named_property(env, result, c"done".as_ptr(), generator_state)
        },
        "Failed to set iterator result done",
    );
    ...
    let catch_result = if argc == 0 {
        ...
        g.catch(
            ...
            Unknown(Value {
                env,
                value: undefined,
                value_type: crate::napi::ValueType::Undefined,
            }),
        )
    } else {
        g.catch(
    ...
    match catch_result {
        Err(e) => {
            ...
            check_status_or_throw!(
                env,
                unsafe {
                    libnode_sys::napi_get_boolean(env, generator_state_value, &mut generator_state)
                },
                "Create generator state failed"
            );
            check_status_or_throw!(
    ...
    match unsafe { ToNapiValue::to_napi_value(env, value) } {
        Ok(val) => {
            check_status_or_throw!(
                env,
                unsafe {
                    libnode_sys::napi_set_named_property(env, result, c"value".as_ptr(), val)
                },
                "Failed to set iterator result value",
            );
        }
