...
impl Object {
    ...
    pub fn get<K: AsRef<str>, V: FromNapiValue>(&self, field: K) -> Result<Option<V>> {
        unsafe {
            self.get_inner(field.as_ref())?
                .map(|v| V::from_napi_value(self.0.env, v))
                .transpose()
        }
    }

    fn get_inner(&self, field: &str) -> Result<Option<libnode_sys::napi_value>> {
    ...
    unsafe fn set_inner(&mut self, field: &str, napi_val: libnode_sys::napi_value) -> Result<()> {
        let c_field = CString::new(field)?;
