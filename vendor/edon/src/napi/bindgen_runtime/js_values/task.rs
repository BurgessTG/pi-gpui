...
extern "C" fn on_abort(
    ...
    unsafe {
        ...
        let reject_status = libnode_sys::napi_reject_deferred(
            env,
            deferred,
            JsError::from(abort_error).into_value(env),
        );
        debug_assert_eq!(
            reject_status,
            libnode_sys::Status::napi_ok,
...
impl<T: Task> ToNapiValue for AsyncTask<T> {
    unsafe fn to_napi_value(
        ...
        if let Some(abort_controller) = val.abort_signal {
            let async_promise =
                async_work::run(env, val.inner, Some(abort_controller.status.clone()))?;
            abort_controller
                .raw_work
