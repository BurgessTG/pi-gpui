...
pub struct Array {
    ...
    inner: libnode_sys::napi_value,
    len: u32,
}

impl Array {
    pub(crate) fn new(env: libnode_sys::napi_env, len: u32) -> Result<Self> {
        let mut ptr = ptr::null_mut();
        unsafe {
            check_status!(
                libnode_sys::napi_create_array_with_length(env, len as usize, &mut ptr),
    ...
    pub fn get<T: FromNapiValue>(&self, index: u32) -> Result<Option<T>> {
        ...
        unsafe {
            check_status!(
                libnode_sys::napi_get_element(self.env, self.inner, index, &mut ret),
    ...
    pub fn set<T: ToNapiValue>(&mut self, index: u32, val: T) -> Result<()> {
        unsafe {
            ...
            check_status!(
                libnode_sys::napi_set_element(self.env, self.inner, index, napi_val),
...
impl FromNapiValue for Array {
    unsafe fn from_napi_value(
        ...
        if is_arr {
            ...
            check_status!(
                unsafe { libnode_sys::napi_get_array_length(env, napi_val, &mut len) },
    ...
    T: FromNapiValue,
{
    unsafe fn validate(
        ...
        check_status!(
            unsafe { libnode_sys::napi_is_array(env, napi_val, &mut is_array) },
            "Failed to check given napi value is array"
        )?;
        if !is_array {
            return Err(Error::new(
