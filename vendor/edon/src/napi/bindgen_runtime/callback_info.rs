...
impl<const N: usize> CallbackInfo<N> {
    ...
    pub fn get_arg(&self, index: usize) -> libnode_sys::napi_value {
        self.args[index]
    }
    ...
    pub fn construct<const IsEmptyStructHint: bool, T: ObjectFinalize + 'static>(
        ...
        self._construct::<IsEmptyStructHint, T>(js_name, obj)
            .map(|(v, _)| v)
    }
    ...
    fn _factory<T: ObjectFinalize + 'static>(
        ...
        if !self.this_reference.is_null() {
            check_status!(
                unsafe {
                    libnode_sys::napi_get_reference_value(self.env, self.this_reference, &mut this)
                },
        ...
        let status = unsafe {
            libnode_sys::napi_new_instance(self.env, this, 0, ptr::null_mut(), &mut instance)
        };
