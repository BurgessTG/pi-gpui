...
impl ThreadsafeFunctionHandle {
    ...
    fn set_raw(&self, raw: libnode_sys::napi_threadsafe_function) {
    ...
    ES: ErrorStrategy::T,
{
    ...
    let val = unsafe {
        match ES::VALUE {
            ...
            ErrorStrategy::Fatal::VALUE => Ok(
                *Box::<ThreadsafeFunctionCallJsBackData<T>>::from_raw(data.cast()),
            ),
        }
    };
        ...
        Ok((values, call_variant, callback)) => {
            ...
            let mut status = match args {
                ...
                Err(e) => match ES::VALUE {
                    ErrorStrategy::Fatal::VALUE => unsafe {
                        libnode_sys::napi_fatal_exception(
                            raw_env,
            ...
            if let ThreadsafeFunctionCallVariant::WithCallback = call_variant {
                ...
                let callback_arg = if status == libnode_sys::Status::napi_pending_exception {
                    ...
                    status = unsafe {
                        libnode_sys::napi_get_and_clear_last_exception(raw_env, &mut exception)
                    };
                    Err(JsUnknown(crate::napi::Value {
                        env: raw_env,
                        value: exception,
            ...
            "Failed to convert return value in ThreadsafeFunction callback into Rust value: {err}"
                    ...
                    unsafe {
                        libnode_sys::napi_fatal_error(
                            c"threadsafe_function.rs:749".as_ptr().cast(),
                            26,
                            c_message.as_ptr(),
...
fn handle_call_js_cb_status(status: libnode_sys::napi_status, raw_env: libnode_sys::napi_env) {
    ...
    if status == libnode_sys::Status::napi_pending_exception {
        ...
        assert!(
            stat == libnode_sys::Status::napi_ok
                || stat == libnode_sys::Status::napi_pending_exception
        );
    } else {
